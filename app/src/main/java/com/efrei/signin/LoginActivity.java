...
import com.efrei.utils.PreferencesUtils;
...
public class LoginActivity extends AppCompatActivity {
    ...
    public void checkUser(){
        ...
        if(!NetworkUtils.isConnected(LoginActivity.this)){
            Toast.makeText(LoginActivity.this, getResources().getString(R.string.login_noNet_message),Toast.LENGTH_SHORT).show();
            return;
        }
        str_username=username.getText().toString();
        str_password=password.getText().toString();
        if(StringUtils.isSpace(str_username)){
            Toast.makeText(LoginActivity.this, getResources().getString(R.string.login_noName_message),Toast.LENGTH_SHORT).show();
            return;
        }
        if(StringUtils.isSpace(str_password)){
            Toast.makeText(LoginActivity.this, getResources().getString(R.string.login_noPW_message),Toast.LENGTH_SHORT).show();
            return;
        }
        /*显示提示正在登录对话框*/
        md=new MaterialDialog.Builder(this)
                .title(getResources().getString(R.string.login_prompt_message))
                .content(getResources().getString(R.string.login_logging_message))
                .progress(true, 0)
                .show();
                ...
                .execute(new StringCallback() {
                    ...
                    public void onSuccess(String s, Call call, Response response) {
                        ...
                        if(login.getStatus().equals(Constant.SUCCESS)){
                            ...
                            Toast.makeText(LoginActivity.this, getResources().getString(R.string.login_success_message),Toast.LENGTH_SHORT).show();
                            LoginActivity.this.finish();
                        }else{
                            if(login.getMsg().equals(Constant.ERROR_SYSTEM)){
                                Toast.makeText(LoginActivity.this, getResources().getString(R.string.login_wrong_message),Toast.LENGTH_SHORT).show();
                                return;
                            }if(login.getMsg().equals(Constant.ERROR_USERNAME)){
                                Toast.makeText(LoginActivity.this, getResources().getString(R.string.login_noUser_message),Toast.LENGTH_SHORT).show();
                                return;
                            }if(login.getMsg().equals(Constant.ERROR_PASSWORD)){
                                Toast.makeText(LoginActivity.this, getResources().getString(R.string.login_wrongPW_message),Toast.LENGTH_SHORT).show();
                                return;
                            }
                        }
                    }
